...
const FALLBACK_RATES = {
  USD:1, EUR:0.92, GBP:0.79, DKK:6.88, SEK:10.41, NOK:10.55, JPY:149.5,
  CNY:7.24, CHF:0.88, CAD:1.36, AUD:1.53, NZD:1.63, HKD:7.82, SGD:1.34,
  KRW:1325, INR:83.1, MXN:17.15, BRL:4.97, ZAR:18.7, RUB:89.5, TRY:30.5,
  PLN:3.97, THB:35.1, IDR:15700, MYR:4.72, PHP:56.5, VND:24500, EGP:30.9,
  SAR:3.75, AED:3.67, ILS:3.68, CZK:22.4, HUF:354, RON:4.58, HRK:6.92,
  BGN:1.80, ISK:137, PKR:278, BDT:110, NGN:773, KES:155, GHS:12.1,
  CLP:900, COP:3950, PEN:3.73, ARS:357, UAH:37.9, TWD:31.5, QAR:3.64,
  KWD:0.308,
};
...
async function fetchRates() {
  ...
  const endpoints = [
    ...
    'https://api.frankfurter.app/latest?from=USD&amount=1',
  ];
...
function updateLastUpdatedText() {
  ...
  if (!el) return;
  if (!state.ratesFetchedAt) { el.textContent = 'using fallback rates'; return; }
...
setInterval(() => { maybeRefreshRates(); }, RATE_REFRESH_MS);
...
function buildCurrencyRow(code, index) {
  ...
  // Determine value
  const value = code === state.baseCode
    ? state.baseAmount
    : convert(state.baseAmount, state.baseCode, code);
...
function startDrag(e, row) {
  ...
  Object.assign(drag, {
    active: true, el: row, ghost,
    pointerOffsetY, startIndex: index, currentIndex: index,
    slots, rows, stepSize,
  });
...
function onDragEnd() {
  ...
  const shiftAtTarget = (drag.startIndex < drag.currentIndex)  ? -drag.stepSize
                      : (drag.startIndex > drag.currentIndex)  ?  drag.stepSize
                      : 0;
  ...
  new p5((p) => {
    ...
    p.draw = function() {
      ...
      pixels.forEach(px => {
        ...
        if (px.y < -10)       { px.y = p.height + 10; px.x = p.random(p.width); }
        if (px.x < -10)       { px.x = p.width  + 10; }
        if (px.x > p.width+10){ px.x = -10; }
      });
    };
