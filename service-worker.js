...
const CACHE_NAME = "cozyfx-v3";
const RATE_CACHE = "cozyfx-rates-v3";
...
self.addEventListener("activate", (event) => {
	event.waitUntil(
		caches
			.keys()
			.then((keys) => {
				return Promise.all(keys.filter((key) => key !== CACHE_NAME && key !== RATE_CACHE).map((key) => caches.delete(key)));
			})
...
async function networkFirstWithCache(request, cacheName) {
	...
	try {
		...
		const cached = await cache.match(request);
		if (cached) return cached;
		return new Response(JSON.stringify({ error: "offline", rates: null }), { status: 503, headers: { "Content-Type": "application/json" } });
	}
}
